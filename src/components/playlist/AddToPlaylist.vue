<template>
    <div id="addToPlaylist" class="playlist-selection">
        <div class="title">
            Выберите плейлисты,<br>
            в которые хотите <strong>добавить</strong><br> 
            этот трек
        </div>
        ...
        <input 
            ...
            @click.prevent="addSongToPlaylists()" 
            class="submit-button" 
            value="Добавить" 
        />     
    </div>
...
export default {
    ...
    methods: {
        ...
        selectPlaylist(selectedPlaylistId) {
            ...
            const playlistsSelection = songCard.querySelector(`#addToPlaylist`);
        ...
        async sendPutRequests() {
            const requests = this.selectedPlaylists.map(playlistId => {
                const url = `http://music.local/api/playlists/${playlistId}/add-song/${this.song.id}`
                return api.put(url);
            });
        ...
        async addSongToPlaylists() { 
            ...
            this.hideModal();
            this.addSongToPlaylistsInComponent();
        },

        addSongToPlaylistsInComponent() {
            this.selectedPlaylists.forEach(playlistId => {
                this.$parent.song.containedInPlaylists.push({ id: playlistId });
            });
